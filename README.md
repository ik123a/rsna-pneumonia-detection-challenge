...
├── app.py                    # Web GUI FastAPI backend server
├── static/                   # Web GUI HTML/CSS/JS frontend assets
├── take_screenshots.js       # Headless Puppeteer screenshot script
...
## 🖥️ Web GUI Clinical Dashboard

An interactive, premium web-based clinical dashboard is available for the RSNA Pneumonia Detection pipeline. It provides an intuitive single-page layout featuring:

* **System Overview & Diagnostics**: Real-time compute device metrics (GPU/CPU, VRAM utilization), loaded model information, and dataset sizes.
* **Diagnostic Tool (Single Inference)**: Drag-and-drop chest X-ray scans (supports DICOM `.dcm` and standard image files like `.png`/`.jpg`), dynamic canvas box overlays, real-time confidence threshold controls, and raw DICOM metadata views.
* **Database Browser**: Browse patient validation files, compare prediction bounding boxes (purple) against ground-truth expert annotations (green dashed) side-by-side, and toggle layers.
* **Training Monitor**: Configure optimizer parameters, batch size, learning rate, and sub-dataset sizes directly, and watch training curves and console logs stream in real-time.
* **Evaluation Curves**: View performance metrics comparisons (Baseline vs Improved) and render curves (PR curve, loss curves, IoU distribution).

### GUI Screenshots

#### 1. System Overview & Diagnostics
![System Overview](overview_tab.png)

#### 2. Single Patient Scan Diagnostics (Diagnostic Tool)
![Diagnostic Tool](inference_tab.png)

#### 3. Database Validation Browser
![Database Browser](browser_tab.png)

#### 4. Interactive Training Monitor
![Training Dashboard](training_tab.png)

#### 5. Model Evaluation & Metrics
![Model Evaluation](evaluation_tab.png)
...
python app.py
```
This spins up the FastAPI backend on `http://127.0.0.1:5000/` and automatically opens the clinical dashboard in your default web browser.
